...
be almost fifty bugs in this system, but nothing else can print The List. Stand in this square, quick! There's no time to explain;
...
The captcha requires you to review a sequence of digits (your puzzle input) and find the sum of all digits that match the next digit in the
...
const solveCaptcha = (input) => {
  ...
  let previousDigit = inputArray[inputArray.length - 1];
  let count = 0;
  inputArray.forEach(digit => {
    if (digit === previousDigit) count += digit;
    previousDigit = digit;
  });
  return count;
}
